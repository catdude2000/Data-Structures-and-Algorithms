...
const count = (target, input) => {
  let count = input.flatMap(entry => entry).filter(flatEntry => flatEntry === target).length || 0;
  return count;
};
...
const totalSum = (input) => {
  return input.flatMap(entry => entry).reduce((fir, sec) => {
    return fir += sec;
  },0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let fin = input.map(onArr => onArr.filter(ent => {
    return typeof ent !== 'string' && ent % 5 === 0;
  })).map(array => array.map(onArr => (Math.pow(2, onArr))));
  return fin;
};
...
let findMaleAndFemale = (data) => {
  let mAF = data.filter(character => /(fe)*male/.test(character.gender)).map(character => character.name).join(' and ');
  return mAF;
};
