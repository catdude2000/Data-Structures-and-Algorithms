...
const containsAnd = (arr) => {
  let newArr3 = arr.filter(element => element.includes('and'));
  return newArr3;
};
...
const oddValues = (arr) => {
  let newArr4 = arr.filter(element => element % 2 );
  return newArr4;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let newArr5 = arr.filter(element => !forbiddenValues.includes(element));
  return newArr5;
};
